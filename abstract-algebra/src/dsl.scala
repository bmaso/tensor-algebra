...
object dsl {
  ...
  def copyTensorElementsToArray[N, ElT <: TensorElT](tensor: Tensor[ElT], arr: Array[N], offset: Int)(implicit ev: ScalaElTMatches[N, ElT]): TensorExpr[ElT, Tensor[ElT]] =
      Free.liftF(CopyTensorElementsToArray(tensor, arr, offset): ({type λ[A] = TensorExprAlg[ElT, A] })#λ[Tensor[ElT]])
