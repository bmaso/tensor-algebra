...
class IntArrayTensorSpec extends FlatSpec {
  ...
  it should "yield expected values from valueAt" in {
    ...
    t.valueAt(Array(2, 0)) should be (10)
    t.valueAt(Array(3, 3)) should be (18)
    t.valueAt(Array(3, 1)) should be (16)
  ...
  "A 7x3x8x2 tensor" should "have expected values from valueAt" in {
    val t = IntArrayTensor((0 to 335) toArray, Array(7, 3, 8, 2), 0, 336)
    t.valueAt(Array(0, 0, 0, 0)) should be (0)
    t.valueAt(Array(2, 0, 0, 0)) should be (96)
    t.valueAt(Array(4, 2, 5, 1)) should be (235)
    t.valueAt(Array(6, 2, 7, 1)) should be (335)
  }
